...
활성 전략은 config의 STRATEGY_NAME(.env)으로 고른다. STRATEGY_NAME 미설정 시
DEFAULT_STRATEGY(golden_cross)로 폴백하고, 나머지는 후보로 등록만 되어 있다
— 전환은 .env의 STRATEGY_NAME 한 줄로 끝난다.
...
[청산(SELL) 조건]
- 별도 매도 진입 신호는 없음(매수 전용). 청산은 아래 트레일링 익절 규칙으로만 판단한다.
- 기준값: 진입가 대비 현재 손익률(pnl_rate)과, 보유 기간 중 도달한 최고 손익률(peak)을
  사용한다. peak는 매 판단 시점까지의 일봉 고가(또는 현재가) 기준 손익률의 최댓값으로
  근사한다. (시스템이 peak를 따로 저장하지 않으므로 일봉 고가로 추정한다.)

- 트레일링 익절(고점을 끌어올리며 추적, 한 번 확보한 이익 구간은 반납하지 않는다):
  · peak < +10%  : 최소 +5% 방어선. 손익률이 +5% 이하로 되밀리면 익절(SELL).
                   아직 +10% 돌파 전이므로 더 큰 상승 여지를 두고 +5%만 지킨다.
  · peak ≥ +10%  : 고점 대비 -3%p 추적 손절. peak 대비 현재 손익률이 3%p 이상
                   되밀리면 익절(SELL).
                   예) peak=+18% → +15% 이탈 시 익절 / peak=+13% → +10% 이탈 시 익절.
  · 위 두 구간 사이에 빈틈이 없도록, +10% 막 돌파 직후부터 곧바로 -3%p 추적이 적용된다.

- 보충: 위 트레일링 외의 하방(예: -10%)은 코드가 강제 손절하므로 여기서 다루지 않는다.
               """),
)
